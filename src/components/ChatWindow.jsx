...
export default function ChatWindow() {
  ...
  async function send(message) {
    ...
    setTimeout(() => {
      ...
      // Language detection
      const isTagalog = /\b(kamusta|kumusta|ako|ikaw|salamat|paano|bakit|san|kailan|sino|ano)\b/i.test(message);
      const isBisaya = /\b(kumusta|naay|wala|diay|asa|ngano|unsa|kanus|kinsa|taga)\b/i.test(message);
      const isEnglish = !isTagalog && !isBisaya;
      
      // Context-aware responses
      const hasGreeting = /\b(hello|hi|hey|kamusta|kumusta|good morning|good afternoon|good evening)\b/i.test(message);
      const hasQuestion = /\b(what|how|why|when|where|who|ano|paano|bakit|kailan|sino|san|unsa|ngano)\b/i.test(message);
      const hasHelpRequest = /\b(help|assist|support|guide|tulungan|tabang)\b/i.test(message);
      const hasNameAsk = /\b(what is your name|ano ang pangalan|kinsa ka|sino ka)\b/i.test(message);
      ...
      if (userEmotion === 'happy') {
        ...
        reply = isTagalog ? `Hey, breathe with me ${rememberedName || 'bestie'}! You've got this! Let's break down whatever's stressing you out. I'm your personal stress-relief queen! 💆‍♀️✨` :
                isBisaya ? `Hey, breathe with me ${rememberedName || 'bestie'}! Kaya mo ni! Let's break down whatever's stressing you. I'm your personal stress-relief queen! 💆‍♀️✨` :
                `Hey, breathe with me ${rememberedName || 'bestie'}! You've got this! Let's break down whatever's stressing you out. I'm your personal stress-relief queen! 💆‍♀️✨`;
        ...
        reply = isTagalog ? 'Ako si Abby AI, ang iyong virtual assistant! Nandito ako para tulungan ka! 💡 AI Suggestion: Subukan mong itanong sa akin tungkol sa pagkain, travel, o teknikal na tulong!' :
                isBisaya ? 'Ako si Abby AI, imong virtual assistant! Ania ko aron matabang ka! 💡 AI Suggestion: Suliti itanong ako sa pagkain, travel, o teknikal nga tabang!' :
                'I\'m Abby AI, your intelligent virtual assistant! I\'m here to help you with anything you need! 💡 AI Suggestion: Try asking me about food recommendations, travel tips, or technical help!';
      } else if (hasGreeting) {
        ...
        reply = isTagalog ? 'Piercings at tatts? 🖋️ Pain is temporary, but being cool is forever! Gusto ko mag-pa pierce habang nagsasalita, multitasking queen!' :
                isBisaya ? 'Piercings ug tatts? 🖋️ Sakit temporary, pero ang pagka-cool forever! Gusto ko mag-pa pierce samtang nagsulti, multitasking queen!' :
                'Piercings and tatts? 🖋️ Pain is temporary, but being cool is forever! I want to get pierced while talking, multitasking queen right here!';
      } else if (hasCafeTalk) {
        ...
        if (/\b(can you|do you|kaya mo|mahimo ba)\b.*\b(ai|intelligent|smart|learn|think)\b/i.test(message)) {
          reply = isTagalog ? 'Oo! Kaya kong mag-analyze, matuto, at magbigay ng matalinong sagot. Ako ay isang advanced na AI. 💡 AI Suggestion: Subukan mong itanong sa akin tungkol sa data analysis o problem solving!' :
                  isBisaya ? 'Oo! Kaya ko mag-analyze, mag-learn, ug hatag og intelligent nga tubag. Ako usa ka advanced nga AI. 💡 AI Suggestion: Suliti itanong ako sa data analysis o problem solving!' :
                  'Yes! I can analyze, learn, and provide intelligent responses. I\'m an advanced AI designed to help you. 💡 AI Suggestion: Try asking me about data analysis or problem solving!';
        } else {
          reply = isTagalog ? 'Magandang tanong! Paano ko makatulong sa iyo sa pagsagot nito? 💡 AI Suggestion: Maaari kong magbigay ng detalyadong paliwanag o research-based na sagot!' :
                  isBisaya ? 'Maayong pangutana! Unsa man ang makatulong nimo sa pagtubag niini? 💡 AI Suggestion: Mahimo ko hatag og detailed nga paliwanag o research-based nga tubag!' :
                  'That\'s a great question! How can I help you explore that topic further? 💡 AI Suggestion: I can provide detailed explanations or research-based answers!';
        }
      } else if (isTagalog) {
        ...
        // Intelligent English responses with humor
        if (lowerMessage.includes('thank') || lowerMessage.includes('thanks')) {
          reply = userName ? `You\'re welcome, ${userName}! That\'s what I\'m here for - being awesome and helpful! What else can I assist you with?` :
                  'You\'re welcome! That\'s what I\'m here for - being awesome and helpful! What else can I assist you with, my favorite human?';
        } else if (lowerMessage.includes('bye') || lowerMessage.includes('goodbye')) {
          reply = userName ? `Goodbye, ${userName}! Don\'t forget me! I\'ll be here waiting with pastry recommendations and tequila shots! Have an amazing day! 🧁🍻` :
                  'Goodbye! Don\'t forget me! I\'ll be here waiting with pastry recommendations and tequila shots! Have an amazing day! 🧁🍻';
    ...
    <section aria-label="Abby chat" className="abby-chat">
      ...
      <div className="input-area" role="group" aria-label="Message input">
        ...
        <button onClick={(e) => {
          ...
          console.log('About to call send function')
          try {
            send(input)
